...
summary<-democracy_by_variable%>%
  ...
  summarise(min=min(value),
            ...
            sd = sd(value),
            max = max(value))
knitr::kable(summary,format="html",align="c")
```
#### d. Histogram for political liberties \n
```{r,warning=FALSE,fig.width=4.8, fig.height=3.6,echo=TRUE,results='asis',message=FALSE}
ggplot(democracy,
       aes(x = POLLIB)) +
...
ggplot(democracy,aes(POLLIB, GDPW))+
  geom_point(position = position_jitter(width = 0.5),alpha=0.35,size=2)+
...
```{r,warning=FALSE,fig.width=4.8,fig.height=3.6,echo=TRUE,message=FALSE}
library(dplyr)
knitr::kable(list(summarize(democracy,
            GDP_mean_all=mean(GDPW)),
            summarize(filter(democracy,CATH>=40),
            GDP_mean_40=mean(GDPW))),format="html",align="c")
```
...
knitr::kable(democracynew%>%
  ...
  summarize(GDP_mean=mean(GDPW)),format="html",align="c")
```

#### m. Median average years of education for all years
```{r,results="asis",warning=FALSE,fig.width=4.8,fig.height=3.6,echo=TRUE,message=FALSE} 
EDT_median1<-democracy%>%
  group_by(YEAR)%>%
  filter(is.na(EDT)==F) %>%
  summarize(EDT_median=median(EDT))
knitr::kable(head(EDT_median1),format="html",align="c")
...
```{r,warning=FALSE,fig.width=6.5,fig.height=3.6,echo=TRUE,message=FALSE} 
EDT_median2<-democracy%>%
  group_by(YEAR,REG)%>%
  filter(is.na(EDT)==F) %>%
  summarize(EDT_median=median(EDT))
knitr::kable(head(EDT_median2),format="html",align="c")

ggplot(democracy, aes(YEAR,EDT)) +
  aes(colour = factor(REG))+
  geom_point(alpha=0.5) +
  stat_summary(fun.y = median, geom="line")+
  scale_color_manual("REG\n",labels = c("democracies", "non-democracies"),values = c("blue", "red"))+
  xlim(c(1955,1990))+
  theme_bw()+
...
#### q. Country closest to the median years of education in 1985 \n
...
knitr::kable(ELF60_p2575,format = "html",align="c")
```
...
These four dataset have almost the same means and standard deviations of x and y, and correlation coefficients between x and y.
...
ggplot(anscombe2, aes(x, y)) +
  ...
  ggtitle("Model 3: Scatter plots of each dataset and its linear regression fits")+
  theme_bw()+
  theme(text=element_text(size=10),
        axis.title=element_text(size=10),
        legend.position="bottom",
...
```{r, echo=FALSE,include=FALSE}
require(RCurl)
sprinters<-read.csv(text=getURL("https://raw.githubusercontent.com/UW-POLS503/Assignment_01/master/sprinters.csv"), header=T)
```
...
```
```{r, results = 'asis'}
htmlreg(list(mod1, mod2, mod3, mod4), caption = "Trends in Winning Times in the Olympic 100-meter dash, 1896-2004",center = FALSE, caption.above = TRUE,vertical.align.px = 0)
```
...
The slop in this regression for men is -0.01. But the slop for women (absolute value) is increasing as year increase -0.02.
...
ggplot(new_Pre1, aes(x = year)) +
  ...
  ggtitle("Model 1: fitted values vs. original values")+
  theme_bw()+
  theme(text=element_text(size=12),
        axis.title=element_text(size=10),
        legend.position="bottom",
...
ggplot(models, aes(x = year)) +
  ...
  geom_line(aes(y = .fitted,colour=factor(women)))+
  scale_color_manual("Gender",labels = c("men", "women"),values = c("blue", "red"))+
  ggtitle("Fig: Fitted values vs. original values for each model")+
  theme_bw()+
  theme(text=element_text(size=12),
        axis.title=element_text(size=10),
        legend.position="bottom",
